...
def render_trending_carousel(video_df, title="🔥 Trending Now", max_items=8):
    """
    Renders a clickable horizontal scrollable carousel using st.components.v1.html.
    Uses JavaScript to detect clicks on the cards and update Streamlit state.
    """
    if video_df.empty:
        return
    ...
    # We need a unique key for this render cycle to pass data back to Streamlit
    if "trending_click_key" not in st.session_state:
        st.session_state.trending_click_key = ""
    ...
    <style>
        ...
        .trending-img {
            ...
            object-fit: cover;
            display: block;
            pointer-events: none; /* Prevents image dragging interfering with click */
        }
        .trending-info {
    ...
    # Build the inner HTML for each card, adding the onclick event
    for _, row in trending_df.iterrows():
        title_clean = row['title'][:50] + ('...' if len(row['title']) > 50 else '')
        video_id = row['video_id']
        
        # The magic line: onclick triggers a JS function that passes the video_id
        html_content += f"""
        <div class="trending-card" onclick="selectVideo('{video_id}')">
    ...
    <!-- JavaScript to bridge the gap between HTML and Streamlit -->
    <script>
        function selectVideo(videoId) {
            // 1. Find the hidden text input generated by Streamlit
            const inputField = document.getElementById('t_input');
            // 2. Update its value with the clicked Video ID
            if(inputField) {
                inputField.value = videoId;
                // 3. Dispatch an input event so Streamlit detects the change
                inputField.dispatchEvent(new Event('input', { bubbles: true }));
                
                // 4. Automatically click the "Load" button we will place next to the input
    ...
    # ============================================================
    # THE HIDDEN STREAMLIT BRIDGE (DO NOT DELETE)
    # ============================================================
    # This hidden input catches the data from the JavaScript above
    hidden_input = st.text_input(
        "Hidden Video Selector", 
        value="", 
        key="trending_click_key", 
        label_visibility="collapsed"
    )
    
    # We style it to be completely invisible
    st.markdown("""
    <style>
    ...
    # If the JavaScript successfully sent a video ID, we trigger the load
    if hidden_input:
        st.session_state.current_video = hidden_input
        # We clear it immediately so it doesn't trigger again on next refresh
        st.session_state.trending_click_key = ""
        st.rerun()
    
    # ============================================================
    # FALLBACK: BUTTONS (For accessibility/old browsers)
    # ============================================================
